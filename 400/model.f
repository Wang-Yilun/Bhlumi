...
c(((((((
! Otionally improved VP with possible shift of VP->VP+error
      KeyPia = MOD(KeyRad,10)
      CALL vacpol(KeyPia,TR,SINW2,RePiE,dRePiE)
      vpfac  = 1/(1 + RePiE)**2
c))))))))
ccccccccc      RePiE  = REPI(TR)
ccccccccc      VPFAC  = 1D0/(1D0+RePiE)**2
...
! half-shift: in the initial state only
      ...
      IF ( KeyZet .EQ. 0) DELZ=0    ! used in main wt
      IF ( KeyZet .EQ. 0) DELZIN=0  ! not used
