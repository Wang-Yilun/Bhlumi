...
!     ***************************
      ...
      COMMON / BHPAR4 / KeyPib ! used in vacpol
      SAVE   
      REAL *4 enq,st2,der,errder,deg,errdeg
      REAL *4 errdersta,errdersys,errdegsta,errdegsys
      parameter (alpha=1.d0/137.035999710d0)
      QQ=Q2
!!!!!!!!!! Standalone Leptonic contributions
c Leptonic VP of Teubner, spacelike QQ<0
c Space-like \Delta\alpha_{lep}^{(5)} = Re\Pi_{lep}^{(5)}
      vprelepsp = alf_lep(1,QQ)+alf_lep(2,QQ)+alf_lep(3,QQ)
c Old approximate leptonic RePi of Burkhardt95
      RePiLep95 = REPIL(QQ)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      IF(KeyPia .EQ. 0) THEN
        RePiE  = 0d0
...
!!!!!!! From Burkhardt and Pietrzyk 1995 (Moriond)
        RePiE  = REPI95(QQ)
        dRePiE = REPI95err(QQ)  ! Interpol. from Table 6 in 1996 workshop
       ELSEIF(KeyPia .EQ. 4) THEN
!!!!!!!!!!!!!!!!!!!!!
!     F. Jegerlehner 2017 version with linked hadr5n17_compact.f
!     F. Jegerlehner 2019 version with linked hadr5x19.f
        enq= QQ/sqrt(abs(QQ))
        st2= SINW2
        CALL hadr5x(enq,st2,der,errdersta,errdersys, deg,errdegsta,errdegsys)
!     combining components
        RePiE  = -der  ! pure hadronic part
ccc        RePiE  = RePiE + RePiLep95       ! Leptonic part from Burkhardt95
        RePiE  = RePiE - vprelepsp       ! Leptonic VP from Teubner
        RePiE  = RePiE - vpretopsp       ! top from Teubner
        dRePiE = SQRT(errdersta**2+errdersys**2) ! Error from hadronic VP
c[[[[[[[[[[[[[[[[
ccc        write(*,*) '@@@@@ from hadr5x: errdersta,errdersys= ', errdersta, errdersys
c]]]]]]]]]]]]]]]]
       ELSEIF(KeyPia .EQ. 5) THEN
!!!!!!! Teubner 2018, linked vp_knr_v3_0.f
        w_input = sqrt(abs(QQ))
ccc       call d_alpha_nores( w_input, a_sp_out, da_sp_out, a_tm_out, da_tm_out )
        CALL d_alpha_reso(  w_input, a_sp_out, da_sp_out, a_tm_out, da_tm_out )  ! the same for Q^2<0
        vprehadsp = alpha*a_sp_out
        vpretopsp = alf_top(QQ)       ! top
!     combining components
        RePiE  = -vprehadsp           ! pure hadronic
ccc        RePiE  =  RePiE  +RePiLep95   ! Leptonic part from Burkhardt95
        RePiE  =  RePiE  - vprelepsp  ! leptonic added
        RePiE  =  RePiE  - vpretopsp  ! top added
        dRePiE =  alpha* da_sp_out    ! error from hadronic
c[[[[[[[[[[[[[[[[
c        write(*,*) '@@@Teunber: QQ,vprehadsp,vprelepsp,pretopsp= ', QQ, vprehadsp, vprelepsp, vpretopsp
c]]]]]]]]]]]]]]]]
       ELSEIF(KeyPia .EQ. 6) THEN
c Davier et.al. 2020
        tran = Q2
       CALL d_alpha(tran,xalpha,xdalpha)
        RePiE  = xalpha
        RePiE  = -xalpha - vprelepsp  ! leptonic added
        RePiE  =  RePiE  - vpretopsp  ! top added
        dRePiE = xdalpha
       ELSE
        ...
        STOP
      ENDIF
      IF( KeyPib .NE. 0 ) RePiE= RePiE +dRePiE
c[[[[[[[[[
c      WRITE(*,*) ' @@@@@@@@@@@@@@@@@@@@@@@@@@@@ vacpol @@@@@@@@@@@@@@@@@@@@@@@@@@@@'
c      WRITE(*,*) ' KeyPia,KeyPib,SINW2= ',KeyPia,KeyPib,SINW2
c      WRITE(*,*) ' Q2= ',Q2
c      WRITE(*,*) ' RePiE,dRePiE= ',RePiE,dRePiE
cc     WRITE(*,*) ' @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
c]]]]]]]]]
      END
...
C    .  ' hadronic part=',HADPI,' total=',REPI95-HADPI
      ...
      FUNCTION REPI95err(S)
C-------------------------------------------- REMARKS ---------------
C Error according to Table 6 in Bhabha workshop 1996
C in the angular range 10-130mrads. Three-point interpolation.
C H.Burkhardt, B.Pietrzyk; see H.Burkhardt in proceedings
C of electroweak Moriond 1995 and Physics Letters.
C--------+---------+---------+---------+---------+---------+---------+--
      IMPLICIT REAL*8(A-H,O-Z)
...
C
      PARAMETER (NREG=12)
      DIMENSION W(NREG),T(NREG)
      DATA W/ 13., 20., 25., 30., 34., 38., 40., 43., 45., 47., 49., 50. /
      ERR = 0d0
      DO I=1,NREG
        theta = 0.020+0.010*(I-1)            ! angle
        T(i) = CMSENE**2 *(1-COS(theta))/2.0 ! positive
      ENDDO
      QQ = -S ! positive
      j=0
      IF( QQ.GE.T(1) .AND. QQ.LE.T(3)  ) j=1
      IF( QQ.GE.T(3) .AND. QQ.LE.T(5)  ) j=3
      IF( QQ.GE.T(5) .AND. QQ.LE.T(7)  ) j=5
      IF( QQ.GE.T(7) .AND. QQ.LE.T(9)  ) j=7
      IF( QQ.GE.T(9) .AND. QQ.LE.T(11) ) j=9
      IF( QQ.GE.T(11).AND. QQ.LE.T(12) ) j=10
      IF( j.GT.0 ) THEN
      ERR = W(j)  * (QQ-T(j+1)) *(QQ-T(j+2)) /(T(J)  -T(j+1)) /(T(J)  -T(j+2))
     $    + W(j+1)* (QQ-T(j  )) *(QQ-T(j+2)) /(T(J+1)-T(j  )) /(T(J+1)-T(j+2))
     $    + W(j+2)* (QQ-T(j  )) *(QQ-T(j+1)) /(T(J+2)-T(j  )) /(T(J+2)-T(j+1))
      ENDIF
      REPI95err = ERR *1d-5
c      WRITE(*,*) '*** REPI95e: QQ= ', QQ, '   J=',j, ' REPI95=',REPI95
      END
...
c[[[[[[[[[[[[[[
c      write(*,*) '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'
c      write(*,*) ' S=', S, ' REPI(S) = ', REPI
c]]]]]]]]]]]]]]
      END

      FUNCTION REPIerr(S)
!-------------------------------------------- REMARKS ---------------
! Error of hadronic VACUUM POLARIZATION in PARAMETRIZATION GIVEN BY
! H.BURKHARDT, TASSO NOTE 192(1981).
! updated see H.Burkhardt et al. Pol. at Lep CERN 88-06 VOL I
!--------------------------------------------------------------------
      IMPLICIT REAL*8(A-H,O-Z)
      ...
      ELSE
        REPI=REPI- (A4+B4*LOG(1.+C4*X))
      ENDIF
      REPIerr = abs(REPI) *0.04d0 ! 4% of hadronic VP
      END
