...
!     ======================     
  ...
  200 CONTINUE
      ievent=ievent+1
      xnevgen5= ievent ! No of crude events for normalization
      WT0=0D0
      WT1=0D0
      WT2=0D0
      WT3=0D0
...
!     **************************** 
     ...
     $BXCLO =  '(1X,15(5H=====)/   )'    )   
      ...
      COMMON / BHPAR4 / KeyPib ! Bypas to vacpol
...
!--
      ...
      KeyPib = 0  ! Can be reset using BHL_SetKeyPib
!--
      AMEL  =  0.5111D-3
      SVAR  = CMSENE**2
      XIMIN = TRMIN/SVAR
...
! ON OUTPUT UNIT NOUT
  ...
  210 SUM(K)=SUM(K)+PHOT(I,K)    
      ...
      WRITE(NOUT,*) 'WTMOD, WTCRU1*WTCRU2 = ', WTMOD, WTCRU1*WTCRU2
...
*******************************************************************************
*******************************************************************************
******   Additional functionality for C++ wrappers
*******************************************************************************
*******************************************************************************
*******************************************************************************
...
!     *******************************
! BORN XSECTION. THA,THB are in radians
! result in nanobarns
!     ***********************************
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      PARAMETER( PI = 3.1415926535897932D0, ALFINV = 137.03604D0)
      ...
      COMMON / BHPAR3 / KeyRad,KeyOpt
      EXTERNAL BORXG
ccc      write(*,*) '%%%% BORNG: CMSENE= ', CMSENE
      KeyPia = MOD(KeyRad,10)
ccc      write(*,*) '%%%% BORNG: KeyPia= ', KeyPia
      XIA= (1D0-DCOS(THA))/2D0
      XIB= (1D0-DCOS(THB))/2D0
      DSIG0 = 4D0*ALFA**2*PI/CMSENE**2*GNANOB
      CALL GAUSJD(BORXG,XIA,XIB,-1D-6,RESULT)
      BORNG= RESULT *DSIG0
      END
      FUNCTION BORXG(XI)
!     ******************
...
!     ***********************************
      ...
      KeyPia = MOD(KeyRad,10)
      tran = CMSENE**2 *XI
      CALL vacpol(KeyPia,-tran,SINW2,RePi2,dRePi2)
      vpfac  = 1/(1 + RePi2)**2
ccc      WRITE(*,*) ' BORXG: KeyPia,tran,SINW2,RePi2,dRePi2,vpfac= ',KeyPia,tran,SINW2,RePi2,dRePi2,vpfac
      BORXG=1D0/XI**2*(1D0+(1D0-XI)**2)/2D0
      IF(KeyPia .GT. 0) BORXG= BORXG *vpfac
      END
...
*/////////////////////////////////////////////////////////////////////////////////////
      ...
      SUBROUTINE BHL_fort_close(nout)
*/////////////////////////////////////////////////////////////////////////////////////
*//   Interface used by c++ programs                                                //
*/////////////////////////////////////////////////////////////////////////////////////
      IMPLICIT NONE
...
*/////////////////////////////////////////////////////////////////////////////////////
*//   No of crude events and crude xsections in nanobarns                           //
*/////////////////////////////////////////////////////////////////////////////////////
      IMPLICIT REAL*8(A-H,O-Z)
      COMMON / BHL5X  / XsPrim5, xnevgen5
      SAVE   / BHL5X  /
      ...
      SUBROUTINE BHL_GetPhoton1(iphot,photk)
*///////////////////////////////////////////////////////////////////////////////
*//                                                                           //
...
*///////////////////////////////////////////////////////////////////////////////
      ...
      INTEGER iphot
      DOUBLE PRECISION   photk(4)
      INTEGER k
*
      DO k=1,4
         photk(k) = PHOT(iphot,k)
      ENDDO
      END
...
*/////////////////////////////////////////////////////////////////////////////////////
      ...
      COMMON / WGTALL / WTMOD,WTCRU1,WTCRU2,WTSET(300)
      SAVE   / WGTALL /
      DOUBLE PRECISION    WtMainX,WtCrudX
      WtMainX = WTMOD          ! the best total weight
      WtCrudX = WTCRU1*WTCRU2  ! Crude weight (helps to avoid bad events)
      END ! KK2f_GetWt


      SUBROUTINE BHL_GetBeams(xp1,xp2)
*/////////////////////////////////////////////////////////////////////////////////////
...
*
      ...
      SUBROUTINE BHL_GetFermions(xq1,xq2)
*/////////////////////////////////////////////////////////////////////////////////////
...
*--------------------------------------------------------------
      IF(j.GE.1 .AND. j.LE. 300) THEN
         WtAlter =   WTCRU1*WTCRU2*WTSET(j)
      ELSE
...
! Reset BHL_KeyPia encoded in KeyRad
!     KeyPia =0,1,2,3 photon vacuum polarization and s-chanel photon
!            =0 OFF, it used in semianalytical tests,
!            =1 ON,  Burkhardt et.al. 1989, as in BHLUMI 2.0x
!            =2 ON,  S. Eidelman, F. Jegerlehner, Z. Phys. C (1995)
...
! Reset BHL_KeyZet encoded in KeyRad
!     KeyPia =0,1 for Z OFF/ON
      IMPLICIT REAL*8(A-H,O-Z)
      COMMON / BHPAR3 / KeyRad,KeyOpt
      KeyPia = MOD(KeyRad,10)
      KeyMod = MOD(KeyRad,100)/10
...
! Reset BHL_KeyMod encoded in KeyRad
!     KeyMod =1, BHLUMI 2.x version of the matrix element
!            =2, BHLUMI 4.x version
      IMPLICIT REAL*8(A-H,O-Z)
      COMMON / BHPAR3 / KeyRad,KeyOpt
      KeyPia = MOD(KeyRad,10)
      KeyMod = MOD(KeyRad,100)/10
...
!
      ...
      COMMON / WGTALL / WTMOD,WTCRU1,WTCRU2,WTSET(300)
!--------------------------------------------------------------------
!     re-Calculate QED Matrix element
      CALL Model(1,WT5)
!--------------------------------------------------------------------
!     Define new Total Principal Weight
      WtMain  = WTCRU1*WTCRU2   *WT5
      END ! BHL_ReModel
